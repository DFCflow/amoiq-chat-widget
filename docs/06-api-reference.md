...
### Widget Bootstrap Authentication

Bootstrap requests use a publishable key together with website-domain resolution:
...
### Widget Runtime Authentication

After bootstrap, widget HTTP and Socket.IO requests should use short-lived gateway JWTs:
...

{
  ...
  "customerToken": "eyJ...",         // Optional: signed identity from your backend
  "userId": "user-456",              // Optional display hint; trusted only with customerToken
...
**User Type Detection:**
- Valid `customerToken` present → trusted logged-in user
- No `customerToken` → anonymous/session-based visitor
...
Connect to the Socket.IO server URL returned by `/webchat/session` or `/webchat/init`.
...
```json
{
  ...
  "ws_token": "eyJ...",
  "widget_token": "eyJ...",
  "websocket_url": "https://websocket.example.com"
}
```
...
4. **Cache widget JWTs** - Reuse the `widget_token` returned by `/webchat/session` or `/webchat/init`
