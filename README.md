...
- **`NEXT_PUBLIC_GATEWAY_API_KEY`** (Optional fallback): Legacy public bootstrap key for local/dev use
  - Browser-side keys are treated as publishable bootstrap credentials, not secrets
  - Gateway resolves tenant/integration from `X-Website-Domain` / `X-Website-Origin`
  - Runtime HTTP and Socket.IO should use the short-lived widget JWTs returned by `/webchat/session` and `/webchat/init`
...
- ✅ **Real-time messaging** - Socket.IO with short-lived JWT authentication
...
- **/embed**: React chat UI (no cache, runs in iframe)
  ...
  - Domain-scoped via gateway lookup; `tenantId` is optional
...
- Publishable bootstrap key + domain validation at Gateway
- Short-lived widget JWTs for HTTP runtime and Socket.IO
- Optional signed `customerToken` for trusted logged-in identity
